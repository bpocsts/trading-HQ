...
import { EmotionChart, MonthlyChart } from '../components/charts/EmotionChart'
import PerformanceChart from '../components/charts/PerformanceChart'
import RecentTrades from '../components/table/RecentTrades'
import {
  QuestsCard, WeeklySummaryCard, HabitTrackerCard,
  EmotionalTrackerCard, MonthlyPerfCard, MistakeTrackerCard,
  AchievementsCard, QuickAccess,
      ...
      <div style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden' }}>
        ...
        <div style={{ padding: '8px 10px', display: 'flex', flexDirection: 'column', ...gap }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.4fr 1fr', ...gap }}>
            <WeeklySummaryCard />
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.22 }} className="glass-card" style={{ padding: 13 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                <div className="card-title" style={{ marginBottom: 0 }}>
                  <span>📈</span>
                  {t('dashboard.performanceOverview')}
                </div>
                <select style={{ background: 'rgba(57,255,20,0.05)', border: '1px solid var(--border)', color: 'var(--ng)', fontSize: 10, padding: '3px 8px', borderRadius: 5, cursor: 'pointer', fontFamily: 'Exo 2', outline: 'none' }}>
                  <option>{t('dashboard.thisWeek')}</option>
                  <option>{t('dashboard.thisMonth')}</option>
                  <option>{t('dashboard.threeMonths')}</option>
                </select>
              </div>
              <PerformanceChart />
            </motion.div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', ...gap }}>
            <MonthlyPerfCard chart={<MonthlyChart data={monthlyData} />} />
            <MistakeTrackerCard />
            <AchievementsCard />
          </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }} style={{ textAlign: 'center', padding: '12px 0 16px', borderTop: '1px solid var(--border)', marginTop: 4 }}>
            ...
            <div style={{ fontSize: 10, color: 'var(--text3)', marginTop: 3 }}>- Ash</div>
          </motion.div>
        </div>
      </div>
    </div>
