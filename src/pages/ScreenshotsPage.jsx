...
export default function ScreenshotsPage() {
  ...
  useEffect(() => {
    if (!user) return
    const q = query(collection(db, 'screenshots'), where('userId', '==', user.uid), orderBy('createdAt', 'desc'))
    const unsub = onSnapshot(q, snap => setScreenshots(snap.docs.map(d => ({ id: d.id, ...d.data() }))))
    return unsub
  }, [user])
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="glass-card" style={{ padding: 16, marginBottom: 16 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr auto', gap: 10, alignItems: 'end' }}>
          <div>
            ...
            <input className="input-dark" placeholder="Setup description..." value={form.caption} onChange={e => setForm(f => ({ ...f, caption: e.target.value }))} />
          </div>
          <div>
        ...
        {screenshots.map((s, i) => (
          ...
          >
            <div style={{ height: 130, background: 'rgba(0,207,255,0.05)', overflow: 'hidden', position: 'relative' }}>
              <img src={s.url} alt={s.caption} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              {s.pair && (
                <div style={{ position: 'absolute', top: 6, left: 6 }}>
                  <span className="tag tag-blue">{s.pair}</span>
                </div>
              )}
            </div>
            <div style={{ padding: '8px 10px' }}>
              <div style={{ fontSize: 10, color: 'var(--text2)', marginBottom: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.caption || 'No caption'}</div>
              <div style={{ fontSize: 9, color: 'var(--text3)' }}>{s.date}</div>
            </div>
          </motion.div>
        ))}
      </div>
